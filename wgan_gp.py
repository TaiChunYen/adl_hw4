...
class FCGenerator(nn.Module):
    def __init__(self, FC_DIM=512):
        ...
        self.relulayer1 = ReLULayer(256, FC_DIM)
...
class GoodGenerator(nn.Module):
    def __init__(self, dim=DIM,output_dim=OUTPUT_DIM):
        ...
        self.ln1 = nn.Linear(256, 1*16*16*self.dim)
        #self.rb1 = ResidualBlock(2*self.dim, 2*self.dim, 3, resample = 'up')
        self.rb2 = ResidualBlock(1*self.dim, 1*self.dim, 3, resample = 'up')#
        self.rb3 = ResidualBlock(1*self.dim, self.dim//2, 3, resample = 'up')
        self.rb4 = ResidualBlock(self.dim//2, self.dim//4, 3, resample = 'up')
        self.bn  = nn.BatchNorm2d(self.dim//4)

        self.conv1 = MyConvo2d(self.dim//4, 3, 3)
        self.relu = nn.ReLU()
        self.tanh = nn.Tanh()

    def forward(self, input):
        output = self.ln1(input.contiguous())
        output = output.view(-1, 1*self.dim, 16, 16)
        #pdb.set_trace()
...
class GoodDiscriminator(nn.Module):
    def __init__(self, dim=DIM, num_class=2):
        ...
        self.conv1 = MyConvo2d(3, self.dim//4, 3, he_init = False)
        self.rb1 = ResidualBlock(self.dim//4, self.dim//2, 3, resample = 'down', hw=DIM)
        self.rb2 = ResidualBlock(self.dim//2, 1*self.dim, 3, resample = 'down', hw=int(DIM/2))
        self.rb3 = ResidualBlock(1*self.dim, 1*self.dim, 3, resample = 'down', hw=int(DIM/4))#
        #self.rb4 = ResidualBlock(2*self.dim, 2*self.dim, 3, resample = 'down', hw=int(DIM/8))
        self.ln1 = nn.Linear(1*16*16*self.dim, 1)

        self.ln2 = nn.Linear(1*16*16*self.dim, self.num_class)
    ...
    def forward(self, input):
        ...
        output = self.rb3(output)
        #output = self.rb4(output)
        output = output.view(-1, 1*16*16*self.dim)
